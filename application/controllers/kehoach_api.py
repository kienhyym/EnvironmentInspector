...
async def kiemtratrung(request=None, Model=None, result=None, **kw):
    check_danhmuc = db.session.query(Model).filter(Model.malinhvuc == data["malinhvuc"]).filter(Model.id != data['id']).first()
...
sqlapimanager.create_api(DanhSachChiNhanhDonVi, max_results_per_page=1000000,
    methods=['GET', 'POST', 'DELETE', 'PUT'],
    url_prefix='/api/v1',
    preprocess=dict(GET_SINGLE=[auth_func], GET_MANY=[auth_func], POST=[auth_func], PUT_SINGLE=[auth_func]),
    postprocess=dict(POST=[], PUT_SINGLE=[], DELETE_SINGLE=[], GET_MANY =[]),
    collection_name='danhsachchinhanhdonvi')

sqlapimanager.create_api(DanhMucLinhVuc, max_results_per_page=1000000,
    methods=['GET', 'POST', 'DELETE', 'PUT'],
    url_prefix='/api/v1',
    preprocess=dict(GET_SINGLE=[auth_func], GET_MANY=[auth_func], POST=[auth_func,kiemtratrung], PUT_SINGLE=[auth_func]),
    postprocess=dict(POST=[], PUT_SINGLE=[], DELETE_SINGLE=[], GET_MANY =[]),
...
sqlapimanager.create_api(CongVanBaoCao, max_results_per_page=1000000,
    methods=['GET', 'POST', 'DELETE', 'PUT'],
    url_prefix='/api/v1',
    preprocess=dict(GET_SINGLE=[auth_func], GET_MANY=[auth_func], POST=[auth_func], PUT_SINGLE=[auth_func]),
    postprocess=dict(POST=[auth_func], PUT_SINGLE=[auth_func], DELETE_SINGLE=[auth_func], GET_MANY =[auth_func]),
    collection_name='congvanbaocao')
